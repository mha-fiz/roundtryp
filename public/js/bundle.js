...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiaGFmaXptaG1kIiwiYSI6ImNrNXl5ZGRoODAxMXUza2xsZXlsb3B0bmEifQ.QSstVxeVPM3YpLKXwaTgRQ';
  var map = new mapboxgl.Map({
    container: 'map',
    //part of html id we want to inject
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; //set marker to the coordinates on data we got from DB

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom' //the part that point to the exact GPS coordinate

    }).setLngLat(loc.coordinates).addTo(map); //Add popup location info

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>".concat(loc.description, "</p>")).addTo(map); //Extend map bound to include the current locations
...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert();
  var alert = "<div class='alert alert--".concat(type, "'>").concat(msg, "</div>"); //insert the alert
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 7:
            ...
            (0, _alert.showAlert)('error', 'Log out failed');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', "".concat(type.toUpperCase(), " successfuly updated!"));
            }
          ...
          case 8:
            ...
            (0, _alert.showAlert)('error', 'Update data failed!');
...
var userPasswordForm = document.querySelector('.form-user-password'); //Display the map
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "49171" + '/');
